...
A single-file biweekly work log for tracking daily activities across pay periods, built for El Paso County Budget Department and shared here for anyone who wants to adapt it.
...
This is a single HTML file — no install, no build step, no server, no account.
...
to a server, and nothing you enter here is visible to anyone else, including
...
your own calendar safely (there's a rollover trap around year boundaries —
...
`AJSU-Tasker_UserGuide.html` — end-user guide, opens in a browser like the app itself.
